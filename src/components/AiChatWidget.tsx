...
export default function AiChatWidget() {
  ...
  return (
    <div className={`fixed z-50 ${open ? 'inset-0 sm:inset-auto sm:bottom-6 sm:right-6' : 'bottom-6 right-6'}`}>
      {open && (
        <div className="w-full h-full sm:mb-3 sm:w-80 sm:rounded-2xl sm:shadow-2xl bg-white sm:border border-gray-200 overflow-hidden flex flex-col sm:h-[420px]">
          {/* Header */}
          <div className="bg-gradient-to-r from-blue-700 to-violet-700 px-4 py-3 sm:px-3 sm:py-2.5 text-white shrink-0 safe-top">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Bot className="w-5 h-5 sm:w-4 sm:h-4" />
                <span className="font-semibold text-sm sm:text-xs">Assistente AI SILCED</span>
              </div>
              <div className="flex items-center gap-1 sm:gap-0.5">
                <button onClick={handleReset} className="p-1.5 sm:p-1 hover:bg-white/20 rounded transition-colors" title="Nuova chat">
                  <RotateCcw className="w-4 h-4 sm:w-3.5 sm:h-3.5" />
                </button>
                <button onClick={() => setOpen(false)} className="p-1.5 sm:p-1 hover:bg-white/20 rounded transition-colors">
          ...
          {/* Messages */}
          <div className="flex-1 overflow-y-auto p-3 sm:p-2.5 space-y-3 sm:space-y-2.5 bg-gray-50">
            {messages.map((msg) => (
              <div key={msg.id}>
                <div className={`flex gap-2 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                  ...
                  <div className={`max-w-[85%] sm:max-w-[80%] rounded-xl px-3 py-2 sm:px-2.5 sm:py-1.5 text-sm sm:text-xs leading-relaxed whitespace-pre-line ${
                    msg.sender === 'user'
                  ...
                  <div className="flex flex-wrap gap-1 mt-1.5 ml-8">
                    {msg.options.map((opt) => (
                      <button
                        ...
                        className="px-3 py-1.5 sm:px-2 sm:py-0.5 bg-white border border-blue-200 text-blue-700 rounded-full text-xs sm:text-[10px] hover:bg-blue-50 transition-colors active:bg-blue-100"
                      >
                        {opt}
                      </button>
                    ))}
          ...
          {/* Input */}
          <div className="p-3 sm:p-2 border-t border-gray-200 bg-white shrink-0 safe-bottom">
            <form onSubmit={(e) => { e.preventDefault(); handleSend(input); }} className="flex gap-2 sm:gap-1.5">
              <input
                type="text"
                value={input}
                onChange={(e) => setInput(e.target.value)}
                placeholder="Scrivi un messaggio..."
                className="flex-1 px-3 py-2.5 sm:px-2.5 sm:py-1.5 border border-gray-200 rounded-xl sm:rounded-lg text-sm sm:text-xs focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-transparent"
              />
              <button
                type="submit"
                disabled={!input.trim()}
                className="p-2.5 sm:p-1.5 bg-blue-600 text-white rounded-xl sm:rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              >
      ...
      {!open && (
        <button
          ...
          className="w-14 h-14 sm:w-12 sm:h-12 bg-gradient-to-br from-blue-600 to-violet-600 text-white rounded-full shadow-lg hover:shadow-xl transition-all hover:scale-110 flex items-center justify-center"
